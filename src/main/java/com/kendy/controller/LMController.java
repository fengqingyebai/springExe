...
//        sumOfEachClubInsure += NumUtil.getNum(record.getSinegleInsurance());
  ...
  private List<GameRecordModel> computSumList(final List<GameRecordModel> list, boolean isNeedSort) {
    ...
    for (Map.Entry<String, List<GameRecordModel>> entry : map.entrySet()) {
      ...
      GameRecordModel firstRecordModel = eachClubList.get(0);
      sumOfEachClubInsure = NumUtil.getNum(firstRecordModel.getClubinsurance());
      GameRecordModel sumRecord = new GameRecordModel();
      sumRecord.setTableid(entry.getKey());
      double singleClubSumYSZJ = 0d;
      boolean isLittleGame = littleGameService.isLittleGame(firstRecordModel);
      if (isLittleGame) {
        singleClubSumYSZJ =  sumOfEachClubZJ * Constants.LITTLE_GAME_RATE_HOU_TAI;
      } else {
        singleClubSumYSZJ = ((sumOfEachClubZJ + sumOfEachClubInsure) * Constants.CURRENT_HS_RATE);
      }
      sumRecord.setYszj(NumUtil
          .digit0("" + singleClubSumYSZJ));
