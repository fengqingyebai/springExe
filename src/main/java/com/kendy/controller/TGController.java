...
//	//=====================================================================
	...
	@FXML private Label tgTeamFwfLabel; // 团队服务费
	...
	public void addNewTGCompanyAction(ActionEvent event) {
		openBasedDialog("TG_add_company_frame.fxml","新增托管公司(当前俱乐部"+MyController.currentClubId.getText()+")"
				,Constants.ADD_COMPANY_FRAME);
	}
	...
	public void refreshTableTGKaixiao() {
		...
		if(CollectUtil.isNullOrEmpty(tgKaixiaoList)) {
			...
			int sum = tgKaixiaoList.stream().mapToInt(info -> NumUtil.getNum(info.getTgKaixiaoMoney()).intValue()).sum();
			sumList.add("开销合计:"+sum);

			typeMap.forEach((name,list) -> {sumList.add(name + ": " 
					+ NumUtil.digit0(list.stream().map(TGKaixiaoInfo::getTgKaixiaoMoney).mapToDouble(NumUtil::getNum).sum())
					);
			});
	...
	public void loadDataLastest() {
		...
		if(CollectUtil.isHaveValue(tgCompanys)) {
			...
			tgCompanys = tgCompanys.stream()
					.filter(info->MyController.currentClubId.getText().equals(info.getClubId())).collect(Collectors.toList());
	...
	private String getTGTeamFwf() {
		...
		if(TableUtil.isHasValue(tableZJSum)) {
			...
			//战绩2.5合计 
			Optional<String> zhaiji25HejiOpt = items.stream().filter(info->"战绩2.5%合计".equals(info.getType())).map(TypeValueInfo::getValue).findFirst();
			//保险合计
			Optional<String> baoxiaoHejiOpt = items.stream().filter(info->"保险合计".equals(info.getType())).map(TypeValueInfo::getValue).findFirst();
	...
	public void refreshTableTGTeamSum() {
		...
		// 1 战绩2.5%：
		double zjRate25Sum = items.stream()
	...
	public void refreshProfitTab() {
		...
		Set<String> proxyTeamSet = tgTeamInfos.stream()
			.filter(info->"1".equals(info.getTgTeamProxy())).map(TGTeamModel::getTgTeamId).collect(Collectors.toSet());
		tgCompanyProxy = tableTGFwf.getItems().stream()
			.filter(info->proxyTeamSet.contains(info.getTgFwfTeamId()))
			.mapToDouble(info->NumUtil.getNum(info.getTgFwfFanshui()) + NumUtil.getNum(info.getTgFwfFanbao())).sum();
	...
	private String getTotalAvailable() {
		...
		Double availabel = NumUtil.getNum(zongLirun) +NumUtil.getNum(yajing) + NumUtil.getNum(zongWaizhai) -  NumUtil.getNum(yiFenhong);
	...
	public void saveCurrentTGCompanyLirnAction(ActionEvent event) {
		String tgCompany = currentTGCompanyLabel.getText();
		if(StringUtil.isBlank(tgCompany)) { ShowUtil.show("请选择托管公司！", 2); return;}
