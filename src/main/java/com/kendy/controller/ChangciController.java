...
public class ChangciController extends BaseController implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle rb) {
    ...
    setTableTotalInfoDialog(stackPane, tableTotalInfo);
   ...
   * 双击第一个表格是展示相应的玩家信息
   */
  public <T> void setTableTotalInfoDialog(StackPane _stackPane, TableView<T> table) {
    table.setOnMouseClicked(e -> {
      // 行双击
      if (e.getClickCount() == 2) {
        TotalInfo item = (TotalInfo) TableUtil.getSelectedRow(table);
        if (item != null && StringUtils.isNotBlank(item.getWanjiaId())) {
          // 具体业务逻辑
          showModalDialog(_stackPane, "玩家信息", getPlayerInfo(item.getWanjiaId()).toString());
        }
      }
    });
  }
