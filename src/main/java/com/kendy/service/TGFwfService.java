...
public class TGFwfService {
	 ...
	 * @param tableTGFwfSum
	 */
	private void setTableTGFwfSumData(TableView<TGFwfinfo>  tableTGFwf, TableView<TypeValueInfo> tableTGFwfSum ,int renci) {
	...
	private List<TGTeamInfo> convert2TGTeamInfo(List<ProxyTeamInfo> proxyTeamInfoList){
		...
		if(CollectUtil.isHaveValue(proxyTeamInfoList)) {
			list = proxyTeamInfoList.stream().map(info -> {
				...
				String teamHuibaoRateStr =  NumUtil.digit2(-Constants.CURRENT_HS_RATE * NumUtil.getNumTimes(tgTeam.getTgBaoxian(), teamHuibaoRateValue) + "");
