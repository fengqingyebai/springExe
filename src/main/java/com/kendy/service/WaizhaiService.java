...
public class WaizhaiService {
  ...
  public Map<String, List<CurrentMoneyInfo>> get_SSJE_Gudong_Map(
    ...
    /*
     * A、负数个人（若其父节点的联合额度为正则不显示 ） B、公司（不显示团队；若其父节点的联合额度为正则不显示，否则不显示子ID,只显示联合额度；其他节点不变
     * C、非公司团队（全部累加到相应团队；若其父节点的联合额度为正则只累加联合额度；其他节点累加实时金额)
     */
    if (MapUtil.isHavaValue(gudongMap)) {
      // LMController.allClubMap.keySet().forEach(clubID ->
      // log.info(LMController.allClubMap.keySet()));
...
//                  pId, _cmSuperIdSum));
            ...
            if (teamInfoOpt.isPresent()) {
              ...
              ite.remove();
              // log.info(String.format("外债：股东%s--有联合ID的父节点%s(%s)将%s转移到%s，并删除父节点",
              // gudongName,playerName,pId,cmi.getShishiJine(),teamInfo.getMingzi()));
            } else {
              // 新增一个所属团队信息
              ite.remove();
...
//                  gudongName, playerName, _teamId, cmi.getCmSuperIdSum()));
          ...
          // 非公司团队，非父节点，累加进其所属的团队中
            ...
            if (teamInfoOpt.isPresent()) {
              ...
              teamInfo.setShishiJine(NumUtil.getSum(teamInfo.getShishiJine(), cmi.getShishiJine()));
              ite.remove();
//              log.info(String.format("外债：股东%s--非父节点%s(%s)将%s转移到%s，并删除非父节点", gudongName, playerName,
//                  pId, cmi.getShishiJine(), teamInfo.getMingzi()));
            } else {
              // 新增一个所属团队信息
              ite.remove();
