...
public class TeamProxyService {
   ...
   * 初始化代理服务类
   */
  public static void initTeamProxy(TableView<ProxyTeamInfo> tableProxyTeam0, HBox proxySumHBox0, // 每列上的总和
   ...
   * @return
   */
  private static String getTeamFWF_GD_EveryDay(String teamId, List<GameRecord> teamEveryDayList,
      Huishui hs) {
  ...
  private static Map<String, List<GameRecord>> copy_Total_Team_Huishui_Map() {
    ...
    // 复制锁定数据(不影响已锁定的数据）
    // 深层复制（代替以上代码）
    for (Map.Entry<String, List<GameRecord>> entry : DataConstans.Total_Team_Huishui_Map
        .entrySet()) {
   ...
   */
  public static Double calculateProxSumFWF(double sumOfHS, double HSRate, double sumOfHB,
   ...
   * 计算服务费 公式 ：服务费 = 总回保 * 回保比例 + 总回水 * 回水比例
   */
  public static Double calculateProxSumFWF(String sumOfHS, String HSRate, String sumOfHB,
      String HBRate) {
   ...
   * @return
   */
  private static String getExportHejiSum(ObservableList<ProxySumInfo> ob_List,
      List<String> filters) {
